...
const chatAttachmentMimeExtMap = {
  "image/jpeg": ".jpg",
  "image/png": ".png",
  "image/webp": ".webp",
  "image/gif": ".gif",
...
const chatUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      ...
      cb(
        null,
        `chat-${Date.now()}-${safeBase || "file"}-${crypto.randomUUID().slice(0, 8)}${mappedExt}`,
      );
    },
  }),
  limits: { fileSize: 8 * 1024 * 1024 },
  fileFilter: (req, file, cb) => {
    if (!chatAttachmentMimeExtMap[file.mimetype]) {
      return cb(new Error("INVALID_CHAT_ATTACHMENT_MIME"));
    }
    return cb(null, true);
  },
});

function withChatUpload(req, res, next) {
  chatUpload.single("attachment")(req, res, (error) => {
    ...
    if (error.message === "INVALID_CHAT_ATTACHMENT_MIME") {
      return res.status(400).json({
        error: "INVALID_INPUT",
...
function buildAttachmentMessage(filename, url) {
  return `附件：${filename}\n${url}`;
}
...
router.post("/upload", requireAuth, withChatUpload, async (req, res) => {
  try {
    const user = req.currentUser;
    const requestedCaseId = req.body?.caseId
      ? String(req.body.caseId).trim()
    ...
    const messageContent = buildAttachmentMessage(
