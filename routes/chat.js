...
const chatUploadDir = path.join(process.cwd(), "uploads", "chat-attachments");
const attachmentMimeExtMap = {
  "image/jpeg": ".jpg",
  "image/png": ".png",
  "image/webp": ".webp",
  "image/gif": ".gif",
...
const chatUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      const mappedExt = attachmentMimeExtMap[file.mimetype] || ".bin";
      const safeBase = String(file.originalname || "attachment")
        .replace(/\.[^/.]+$/, "")
        .replace(/[^a-zA-Z0-9-_]/g, "-")
        .slice(0, 40);
      const filename = `chat-${Date.now()}-${safeBase || "file"}-${crypto.randomUUID().slice(0, 8)}${mappedExt}`;
      cb(null, filename);
    },
  }),
  limits: { fileSize: 8 * 1024 * 1024 },
  fileFilter: (req, file, cb) => {
    if (!attachmentMimeExtMap[file.mimetype]) {
      return cb(new Error("INVALID_ATTACHMENT_MIME"));
    }
    return cb(null, true);
  },
});

function withChatAttachmentUpload(req, res, next) {
  chatUpload.single("attachment")(req, res, (error) => {
    ...
    if (
      ...
      return res.status(400).json({
        error: "INVALID_INPUT",
        message: "附件大小不可超過 8MB",
      });
    }

    if (error.message === "INVALID_ATTACHMENT_MIME") {
      return res.status(400).json({
        error: "INVALID_INPUT",
        message: "附件格式僅支援 JPG、PNG、WEBP、GIF、PDF、TXT",
      });
    }
...
function buildAttachmentContent({ filename, url }) {
  return `附件：${filename}\n${url}`;
}

async function findConsultationByCaseNoForSupport(caseNo) {
  const [rows] = await pool.execute(
    `
      SELECT id, user_id AS userId, case_no AS caseNo, status
    ...
    try {
      ...
      if (caseId) {
        ...
        const [result] = await pool.execute(
          ...
          VALUES (?, ?, ?, 'OPEN', 'NONE', NOW(), NOW(), NOW(), NOW())
        `,
          [caseId, user.id, `附件-${req.file.originalname}`.slice(0, 40)],
        );
        consultationId = result.insertId;
      }

      const attachmentUrl = `${req.protocol}://${req.get("host")}/uploads/chat-attachments/${req.file.filename}`;
      const messageContent = buildAttachmentContent({
        filename: req.file.originalname,
        url: attachmentUrl,
      });
      const messageMeta = JSON.stringify({
        caseId,
        event: "MESSAGE",
        attachment: {
      ...
      `,
        [consultationId, user.id, messageContent, messageMeta],
      );
      ...
      emitCaseMessage({ userId: user.id, caseId, message });
      emitCaseUpdated({ userId: user.id, caseId, status: "OPEN" });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const consultation = await findConsultationByCaseNoForSupport(caseId);
      if (!consultation) {
        return res.status(404).json({ error: "CASE_NOT_FOUND" });
      }
      ...
      const [insertResult] = await pool.execute(
        ...
        INSERT INTO chat_messages
        (consultation_id, user_id, content, sender, type, metadata, created_at)
        VALUES (?, ?, ?, 'SYSTEM', 'FILE', ?, NOW())
      `,
        [consultation.id, supportUser.id, messageContent, supportMeta],
      );
      ...
      console.error("[chat/support/upload]", error);
      return res.status(500).json({ error: "INTERNAL_ERROR" });
    }
  },
);
